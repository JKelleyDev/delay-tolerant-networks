...
class SatellitePhysicalLayer:
    ...
    def calculate_free_space_path_loss(self, distance_km: float) -> float:
        ...
        """
        return (
            20 * math.log10(distance_km) + 20 * math.log10(self.frequency_ghz) + 92.45
        )
    ...
    def estimate_signal_to_noise_ratio(
        ...
        noise_power_db = 10 * math.log10(
            self.boltzmann_const * self.noise_temperature_k * self.bandwidth_hz
        )
    ...
    def calculate_link_capacity(
        ...
        snr = self.estimate_signal_to_noise_ratio(
            tx_power_w, tx_gain_db, rx_gain_db, path_loss
        )
    ...
    def calculate_bit_error_rate(
        self, snr_linear: float, modulation_type: str = "QPSK"
    ) -> float:
        """
