...
class TestContactPrediction(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Keep gs_pos for other tests that need it
        self.gs_pos = self.gs.to_ecef_position()

        # Create simple test positions that should definitely be above/below horizon
        # Use realistic ECI coordinates (km scale)

        # For a satellite clearly above horizon: place it high above Earth's surface
        self.sat_above = Position3D(
            1000.0, 1000.0, 7000.0, "ECI"  # ~600km altitude, should be visible
        )

        # For a satellite clearly below horizon: place it inside Earth
        self.sat_below = Position3D(
            100.0, 100.0, 1000.0, "ECI"  # Much lower than Earth's surface
        )
    ...
    def test_polar_station_high_inclination_satellite(self):
        ...
        # Use longer time window (6 hours) and smaller time step for better detection
        windows = self.predictor.predict_contact_windows(
            elements, "POLAR", "svalbard", current_time, 6.0, time_step_seconds=10
        )
        self.assertGreaterEqual(
            len(windows), 1, "Should detect at least one contact window in 6 hours"
        )
