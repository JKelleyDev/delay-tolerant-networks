...
def test_transmission_time_and_fit(phy):
    ...
    tx_time = phy.estimate_transmission_time(
        bundle_bytes, cap_bps, contact_duration_sec=3600.0
    )
    assert tx_time > 0.0 and tx_time != float("inf")
    # very small capacity should not fit -> returns inf
    tx_time_impossible = phy.estimate_transmission_time(
        bundle_bytes, link_capacity_bps=1.0, contact_duration_sec=10.0
    )
...
def test_doppler_shift(phy):
    ...
    shift = phy.estimate_doppler_shift(
        relative_velocity_mps=1000.0, carrier_frequency_hz=carrier
    )
