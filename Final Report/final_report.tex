...
\subsection{DTN-Specific Application Features}
...
\begin{lstlisting}[language=Python]
def calculate\_forwarding\_priority(self, bundle, contact):
    """Priority based on link quality and routing algorithm"""
    base\_priority = contact.data\_rate / 100.0

    # Boost for direct delivery
    if contact.target\_id == bundle.destination.ssp:
        base\_priority += 10.0

    # Reduce for poor link quality
    if contact.snr\_db < 15.0:
        base\_priority *= 0.5

    return base\_priority
\end{lstlisting}
...
\begin{lstlisting}[language=Python]
def can\_transmit\_bundle(self, bundle, contact\_window):
    """Check if bundle can be transmitted in available time"""
    transmission\_time = bundle.payload\_size / contact\_window.data\_rate
    remaining\_time = contact\_window.remaining\_duration()

    return transmission\_time <= remaining\_time
\end{lstlisting}
...
\begin{lstlisting}[language=Python]
def update\_routing\_info(self, contact\_plan):
    """Update routing state with predicted contacts"""
    for contact in contact\_plan:
        if self.is\_neighbor\_contact(contact):
            # Update delivery predictability for PRoPHET
            self.\_update\_encounter\_predictability(
                contact.target\_id, contact.start\_time
            )
\end{lstlisting}
